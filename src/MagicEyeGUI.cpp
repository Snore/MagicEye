...
{
    int currentIndex = 0;
    for (auto resultsBB_itr = _resultCardsROIBounds.cbegin(); resultsBB_itr != _resultCardsROIBounds.cend();
         ++resultsBB_itr)
    {
...
void MagicEyeGUI::initializeROIs()
{
    ...
    _resultsTextROIBounds =
        cv::Rect(0, _mainDisplayROIBounds.height - TEXT_HEIGHT, _mainDisplayROIBounds.width, TEXT_HEIGHT);
