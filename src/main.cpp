...
void compileMetrics()
{
    ...
    metrics << cardsCorrect_FirstGuess << " completely correct cards and " << cardsCorrect_TopSeven
            << " top seven correct cards.\n";
    ...
    metrics << cardsIncorrect_RightPartial << " completely correct partial cards and " << cardsIncorrect_ConfusedPartial
            << " color confused cards.\n";
...
void mouseEventCallback(int mouseEvent, int x, int y, [[maybe_unused]] int flags, [[maybe_unused]] void *userData)
{
    ...
    while (true)
    {
        if (!pause)
        {
            ...
            else
            {
                ...
                // cardFinder.discernPartialCards(); // needs to be called after frame colors are assigned.  TODO start
                // here
