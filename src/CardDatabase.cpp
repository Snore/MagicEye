...
std::vector<MagicCard *> CardDatabase::returnMostAlike(MagicCard const *const cardToMatch_ptr,
                                                       const int groupSize) const
{
    ...
    for (auto itr = std::make_pair(remainingList.cbegin(), matchesValues.begin()); itr.first != remainingList.cend();
...
void CardDatabase::initializeCardFrameHistograms()
{
    ...
    const int histogramSizes[] = {CardMeasurements::HueBins,
                                  CardMeasurements::SaturationBins,
                                  CardMeasurements::ValueBins};
    ...
    std::vector<std::string> greenSamplePaths = {ASSETS_PATH + "\\FrameTrainingImages\\green\\abundant growth.jpg",
                                                 ...
                                                 ASSETS_PATH +
                                                     "\\FrameTrainingImages\\green\\beastmaster ascension.jpg",
    ...
    for (std::vector<std::string>::const_iterator greenSamplePathItr = greenSamplePaths.cbegin();
         greenSamplePathItr != greenSamplePaths.cend();
         ++greenSamplePathItr)
    {
        MagicCard card(*greenSamplePathItr);
        cv::addWeighted(_greenFrameHistogram,
                        1.0,
                        card.getFrameHistogram(),
                        accumulationRatio,
                        0.0,
                        _greenFrameHistogram);
        cv::addWeighted(_greenFrameMean_CIELAB,
                        1.0,
                        card.getFrameMeanColor_CIELAB(),
                        accumulationRatio,
                        0.0,
                        _greenFrameMean_CIELAB);
    }
    std::cout << "Green card mean point: X:" << _greenFrameMean_CIELAB[0] << " Y:" << _greenFrameMean_CIELAB[1]
              << " Z:" << _greenFrameMean_CIELAB[2] << "\n";
    ...
    for (std::vector<std::string>::const_iterator redSamplePathItr = redSamplePaths.cbegin();
         redSamplePathItr != redSamplePaths.cend();
         ++redSamplePathItr)
    {
        ...
        cv::addWeighted(_redFrameMean_CIELAB,
                        1.0,
                        card.getFrameMeanColor_CIELAB(),
                        accumulationRatio,
                        0.0,
                        _redFrameMean_CIELAB);
    }
    std::cout << "Red card mean point: X:" << _redFrameMean_CIELAB[0] << " Y:" << _redFrameMean_CIELAB[1]
              << " Z:" << _redFrameMean_CIELAB[2] << "\n";
    ...
    for (std::vector<std::string>::const_iterator blueSamplePathItr = blueSamplePaths.cbegin();
         blueSamplePathItr != blueSamplePaths.cend();
         ++blueSamplePathItr)
    {
        MagicCard card(*blueSamplePathItr);
        cv::addWeighted(_blueFrameHistogram,
                        1.0,
                        card.getFrameHistogram(),
                        accumulationRatio,
                        0.0,
                        _blueFrameHistogram);
        cv::addWeighted(_blueFrameMean_CIELAB,
                        1.0,
                        card.getFrameMeanColor_CIELAB(),
                        accumulationRatio,
                        0.0,
                        _blueFrameMean_CIELAB);
    }
    std::cout << "Blue card mean point: X:" << _blueFrameMean_CIELAB[0] << " Y:" << _blueFrameMean_CIELAB[1]
              << " Z:" << _blueFrameMean_CIELAB[2] << "\n";
    ...
    for (std::vector<std::string>::const_iterator whiteSamplePathItr = whiteSamplePaths.cbegin();
         whiteSamplePathItr != whiteSamplePaths.cend();
         ++whiteSamplePathItr)
    {
        MagicCard card(*whiteSamplePathItr);
        cv::addWeighted(_whiteFrameHistogram,
                        1.0,
                        card.getFrameHistogram(),
                        accumulationRatio,
                        0.0,
                        _whiteFrameHistogram);
        cv::addWeighted(_whiteFrameMean_CIELAB,
                        1.0,
                        card.getFrameMeanColor_CIELAB(),
                        accumulationRatio,
                        0.0,
                        _whiteFrameMean_CIELAB);
    }
    std::cout << "White card mean point: X:" << _whiteFrameMean_CIELAB[0] << " Y:" << _whiteFrameMean_CIELAB[1]
              << " Z:" << _whiteFrameMean_CIELAB[2] << "\n";
    ...
    for (std::vector<std::string>::const_iterator blackSamplePathItr = blackSamplePaths.cbegin();
         blackSamplePathItr != blackSamplePaths.cend();
         ++blackSamplePathItr)
    {
        MagicCard card(*blackSamplePathItr);
        cv::addWeighted(_blackFrameHistogram,
                        1.0,
                        card.getFrameHistogram(),
                        accumulationRatio,
                        0.0,
                        _blackFrameHistogram);
        cv::addWeighted(_blackFrameMean_CIELAB,
                        1.0,
                        card.getFrameMeanColor_CIELAB(),
                        accumulationRatio,
                        0.0,
                        _blackFrameMean_CIELAB);
    }
    std::cout << "Black card mean point: X:" << _blackFrameMean_CIELAB[0] << " Y:" << _blackFrameMean_CIELAB[1]
              << " Z:" << _blackFrameMean_CIELAB[2] << "\n";

    // Multi-color frame samples
    std::vector<std::string> multiSamplePaths = {
        ASSETS_PATH + "\\FrameTrainingImages\\yellow\\bruna, light of alabaster.jpg",
    ...
    for (std::vector<std::string>::const_iterator multiSamplePathItr = multiSamplePaths.cbegin();
         multiSamplePathItr != multiSamplePaths.cend();
         ++multiSamplePathItr)
    {
        MagicCard card(*multiSamplePathItr);
        cv::addWeighted(_yellowFrameHistogram,
                        1.0,
                        card.getFrameHistogram(),
                        accumulationRatio,
                        0.0,
                        _yellowFrameHistogram);
        cv::addWeighted(_yellowFrameMean_CIELAB,
                        1.0,
                        card.getFrameMeanColor_CIELAB(),
                        accumulationRatio,
                        0.0,
                        _yellowFrameMean_CIELAB);
    }
    std::cout << "Multi card mean point: X:" << _yellowFrameMean_CIELAB[0] << " Y:" << _yellowFrameMean_CIELAB[1]
              << " Z:" << _yellowFrameMean_CIELAB[2] << "\n";

    // Colorless frame samples
    std::vector<std::string> colorlessSamplePaths = {
        ASSETS_PATH + "\\FrameTrainingImages\\artifact\\darksteel brute.jpg",
    ...
    for (std::vector<std::string>::const_iterator colorlessSamplePathItr = colorlessSamplePaths.cbegin();
         colorlessSamplePathItr != colorlessSamplePaths.cend();
         ++colorlessSamplePathItr)
    {
        MagicCard card(*colorlessSamplePathItr);
        cv::addWeighted(_artifactFrameHistogram,
                        1.0,
                        card.getFrameHistogram(),
                        accumulationRatio,
                        0.0,
                        _artifactFrameHistogram);
        cv::addWeighted(_artifactFrameMean_CIELAB,
                        1.0,
                        card.getFrameMeanColor_CIELAB(),
                        accumulationRatio,
                        0.0,
                        _artifactFrameMean_CIELAB);
    }
    std::cout << "Colorless card mean point: X:" << _artifactFrameMean_CIELAB[0]
              << " Y:" << _artifactFrameMean_CIELAB[1] << " Z:" << _artifactFrameMean_CIELAB[2] << "\n";
    ...
    for (std::vector<std::string>::const_iterator landSamplePathItr = landSamplePaths.cbegin();
         landSamplePathItr != landSamplePaths.cend();
         ++landSamplePathItr)
    {
        MagicCard card(*landSamplePathItr);
        cv::addWeighted(_landFrameHistogram,
                        1.0,
                        card.getFrameHistogram(),
                        accumulationRatio,
                        0.0,
                        _landFrameHistogram);
        cv::addWeighted(_landFrameMean_CIELAB,
                        1.0,
                        card.getFrameMeanColor_CIELAB(),
                        accumulationRatio,
                        0.0,
                        _landFrameMean_CIELAB);
    }
    std::cout << "Land card mean point: X:" << _landFrameMean_CIELAB[0] << " Y:" << _landFrameMean_CIELAB[1]
              << " Z:" << _landFrameMean_CIELAB[2] << "\n";
...
std::vector<MagicCard *> CardDatabase::selectFrameColorFrom(const std::vector<MagicCard *> &fromCards,
                                                            const CardDetails::FrameColor selectColor) const
{
...
std::vector<MagicCard *> CardDatabase::selectSetFrom(const std::vector<MagicCard *> &fromCards,
                                                     const CardDetails::CardSet selectSet) const
{
...
CardDetails::FrameColor CardDatabase::getLiveCardColor(const MagicCard* card) const
{
    ...
    if (bestMatchingColor == CardDetails::Colorless || bestMatchingColor == CardDetails::White || bestMatchingColor ==
CardDetails::Black)
    {
        ...
        // Colorless
        const double colorlessChromaFrameDistance = cv::sqrt(cv::pow((cardChroma - _artifactFrameMean_CIELAB_Live[0]),
2)); if (colorlessChromaFrameDistance < bestMatchingDistance)
        {
            bestMatchingColor = CardDetails::Colorless;
...
void CardDatabase::trainLiveCardColor(const MagicCard* card, const CardDetails::FrameColor fcolor)
{
    ...
    case CardDetails::White:
        cv::addWeighted(_whiteFrameMean_CIELAB_Live, 0.5, cardFrameMean_CIELAB, 0.5, 0.0, _whiteFrameMean_CIELAB_Live);
// Change to this way if the training is too slow
        //cv::addWeighted(_whiteFrameMean_CIELAB_Live, 1.0, cardFrameMean_CIELAB, (1 /
++_whiteFrameMean_CIELAB_Live_Samples), 0.0, _whiteFrameMean_CIELAB_Live); std::cout << "card frame CIELAB mean point
for live white frame: X:" << _whiteFrameMean_CIELAB_Live[0] << " Y:" << _whiteFrameMean_CIELAB_Live[1] << " Z:" <<
_whiteFrameMean_CIELAB_Live[2] << "\n";  // DEBUG break; case CardDetails::Black:
        cv::addWeighted(_blackFrameMean_CIELAB_Live, 0.5, cardFrameMean_CIELAB, 0.5, 0.0, _blackFrameMean_CIELAB_Live);
        std::cout << "card frame CIELAB mean point for live black frame: X:" << _blackFrameMean_CIELAB_Live[0] << " Y:"
<< _blackFrameMean_CIELAB_Live[1] << " Z:" << _blackFrameMean_CIELAB_Live[2] << "\n";  // DEBUG break; case
CardDetails::Red: cv::addWeighted(_redFrameMean_CIELAB_Live, 0.5, cardFrameMean_CIELAB, 0.5, 0.0,
_redFrameMean_CIELAB_Live); std::cout << "card frame CIELAB mean point for live red frame: X:" <<
_redFrameMean_CIELAB_Live[0] << " Y:" << _redFrameMean_CIELAB_Live[1] << " Z:" << _redFrameMean_CIELAB_Live[2] << "\n";
// DEBUG break; case CardDetails::Blue: cv::addWeighted(_blueFrameMean_CIELAB_Live, 0.5, cardFrameMean_CIELAB, 0.5, 0.0,
_blueFrameMean_CIELAB_Live); std::cout << "card frame CIELAB mean point for live blue frame: X:" <<
_blueFrameMean_CIELAB_Live[0] << " Y:" << _blueFrameMean_CIELAB_Live[1] << " Z:" << _blueFrameMean_CIELAB_Live[2] <<
"\n";  // DEBUG break; case CardDetails::Green: cv::addWeighted(_greenFrameMean_CIELAB_Live, 0.5, cardFrameMean_CIELAB,
0.5, 0.0, _greenFrameMean_CIELAB_Live); std::cout << "card frame CIELAB mean point for live green frame: X:" <<
_greenFrameMean_CIELAB_Live[0] << " Y:" << _greenFrameMean_CIELAB_Live[1] << " Z:" << _greenFrameMean_CIELAB_Live[2] <<
"\n";  // DEBUG break; case CardDetails::Colorless: cv::addWeighted(_artifactFrameMean_CIELAB_Live, 0.5,
cardFrameMean_CIELAB, 0.5, 0.0, _artifactFrameMean_CIELAB_Live); std::cout << "card frame CIELAB mean point for live
colorless frame: X:" << _artifactFrameMean_CIELAB_Live[0] << " Y:" << _artifactFrameMean_CIELAB_Live[1] << " Z:" <<
_artifactFrameMean_CIELAB_Live[2] << "\n";  // DEBUG break; case CardDetails::Multi:
        cv::addWeighted(_yellowFrameMean_CIELAB_Live, 0.5, cardFrameMean_CIELAB, 0.5, 0.0,
_yellowFrameMean_CIELAB_Live); std::cout << "card frame CIELAB mean point for live multicolor frame: X:" <<
_yellowFrameMean_CIELAB_Live[0] << " Y:" << _yellowFrameMean_CIELAB_Live[1] << " Z:" << _yellowFrameMean_CIELAB_Live[2]
<< "\n";  // DEBUG break; default:
        // Not a valid color?
        assert(false);
        break;
...
double CardDatabase::calcDeltaE(const cv::Scalar colorPointOne, const cv::Scalar colorPointTwo) const
{
    ...
    // CIE94 - better, but not yet perfect  /// can add this back in after video.  Can always omit L_culmination if want
    // to drop luminosity like above
    ...
    // const double delta_Hstar_ab = cv::sqrt((delta_astar * delta_astar) + (delta_bstar * delta_bstar) -
    // (delta_Cstar_ab * delta_Cstar_ab)); const double kL = 1; // graphic arts /// 2.0 // textiles const double K_one =
    // 0.045; // graphic arts /// 0.048 // textiles const double K_two = 0.015; // graphic arts /// 0.014 // textiles
    // const double Sl = 1.0;
    ...
    //
    //// coup de grace
    // const double euclidianDistance = cv::sqrt((L_culmination * L_culmination) + (C_culmination * C_culmination) +
    // (h_culmination *h_culmination));
...
CardDetails::FrameColor CardDatabase::getLiveCardColor(const MagicCard *card) const
{
    ...
    // Colorless
    // const double colorlessFrameDistance = cv::compareHist(cardFrameHistogram, _artifactFrameHistogram,
    // COMP_HIST_METHOD);
    const double colorlessFrameDistance =
        cv::compareHist(_artifactFrameHistogram, cardFrameHistogram, COMP_HIST_METHOD);
    if (colorlessFrameDistance < bestMatchingDistance)
    {
        bestMatchingColor = CardDetails::Colorless;
        bestMatchingDistance = colorlessFrameDistance;
...
void CardDatabase::trainLiveCardColor(const MagicCard *card, const CardDetails::FrameColor fcolor)
{
    ...
    case CardDetails::White:
        // Change to this way if the training is too slow
        cv::addWeighted(_whiteFrameHistogram, 0.5, cardFrameHistogram, 0.5, 0.0, _whiteFrameHistogram);
        break;
