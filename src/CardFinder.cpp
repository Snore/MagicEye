...
CardFinder::CardFinder(CardDatabase *const cdb) : _cdb_ptr(cdb)
{
}
...
std::vector<TableCard> *CardFinder::findAllCards(cv::Mat &scene)
{
    ...
    /// ???
    /// Profit
    //_lastFrameCards = _foundCards;																						// TODO maybe make vector
    //of pointers
    rememberNewCards();
    forgetOldCards();
    _biggestBox = cv::Rect();
    ...
    // cv::imshow("trash", tableTopBinary);  // Show the bin mask of all objects that are on the table
    // // Debug
    ...
    {
        // full cards are megenta, all other cards are yellow
        outlineRotatedRectangle(
            scene,
            MC_itr->getMinimumBoundingRect(),
            (MC_itr->getCardVisibility() == TableCard::Visible ? CV_RGB(255, 0, 255) : CV_RGB(0, 255, 255)));
    }
                                       ...
                                       std::vector<TableCard> &runningList) const
{
    ...
    cv::Mat bwScene;
    cv::cvtColor(ROI, bwScene, cv::COLOR_BGR2GRAY); // OR: Now filter the black regions by filtering the HSV Range V-
                                                    // 100 to 255. The result will look like this.
    ...
    cv::threshold(bwScene,
                  bwScene,
                  80,
                  255,
                  cv::THRESH_BINARY_INV); // | CV_THRESH_OTSU); // was 42 // 70 for mid day shots
    ...
    while (!contours.empty()) // While there are still contours to process
    {
        ...
        */
        ...
        // remove all contours that are completely enclosed by the bounding rectangle enclosing the largest contour,
        // including the largest contour this will remove noise that is part of a card
        contours.erase(
            biggestContourFound); // remove this now because once we remove one object this iterator is invalidated.
                                       ...
                                       const bool isFullCard) const
{
    ...
    // croppedToFit = rotatedUpright(cv::Rect(cardCenter.x - (boxSize.width / 2), cardCenter.y - (boxSize.height / 2),
    // boxSize.width, boxSize.height));
    cv::getRectSubPix(rotatedUpright, boxSize, cardCenter /*boundingRect.center*/, croppedToFit);

    // return
    const cv::Point2f newCenter(boundingRect.center.x + static_cast<float>(worldPosition.x),
                                boundingRect.center.y + static_cast<float>(worldPosition.y));
    const cv::RotatedRect cardBoundingBox(newCenter, boundingRect.size, boundingRect.angle);
    const TableCard::VisibilityState cardVisibility =
        (isFullCard ? TableCard::Visible : TableCard::PartialBlockedUnidentified);
    return TableCard(cardBoundingBox, croppedToFit, cardVisibility);
}

bool CardFinder::isContourConsumedByAnother(const std::vector<cv::Point> contour,
                                            const std::vector<cv::Point2f> consumedBy) const
{
    ...
    {
        // if at least one point lies outside of the enclosing contour...
        if (cv::pointPolygonTest(consumedBy, *point_itr, false) == -1)
        {
...
cv::Mat CardFinder::findPlayField(const cv::Mat &scene) const
{
    ...
    cv::floodFill(_channels[1],
                  backgroundMask,
                  centerPoint,
                  CV_RGB(UCHAR_MAX, UCHAR_MAX, UCHAR_MAX),
                  NULL,
                  cv::Scalar(4.0),
                  cv::Scalar(3.0),
...
void CardFinder::forgetOldCards()
{
    ...
    while (memoryCards_itr != _cardMemory.end())
    {
        ...
        {
            // if the card is contained under the biggest box, then it should not be forgotten yet since it's obstructed
        by the payer's hand
...
void CardFinder::discernPartialCards()
{
    ...
    for (auto foundCard_itr = _foundCards.begin(); foundCard_itr != _foundCards.end(); ++foundCard_itr)
    {
        ...
        {
            // std::remove_if(potentialCards.begin(), potentialCards.end(), [foundCard_itr](TableCard pot_tableCard) {
            // return foundCard_itr->isProbablySameTableCard(pot_tableCard); });
    ...
    // step 3
    for (auto foundCardUnknown_itr = _foundCards.begin(); foundCardUnknown_itr != _foundCards.end();
         ++foundCardUnknown_itr)
    {
        if (foundCardUnknown_itr->getCardVisibility() == TableCard::PartialBlockedUnidentified)
        {
            ...
            for (auto recallCard_itr = _cardMemory.begin(); recallCard_itr != _cardMemory.end(); ++recallCard_itr)
            {
                if (recallCard_itr->getCardVisibility() == TableCard::PartialBlocked &&
                    (recallCard_itr->hasBeenIdentified == false) &&
                    recallCard_itr->isProbablySameTableCard(*foundCardUnknown_itr))
                {
                    recallCard_itr->resetTimedReferenceCheck();
         ...
         ++foundCardUnknown_itr)
    {
        if (foundCardUnknown_itr->getCardVisibility() == TableCard::PartialBlockedUnidentified)
        {
            ...
            for (auto colorMatcher_itr = potentialCardIndicies.begin(); colorMatcher_itr != potentialCardIndicies.end();
                 ++colorMatcher_itr)
            {
                if (foundCardUnknown_itr->getCardFrameColor() == _cardMemory[(*colorMatcher_itr)].getCardFrameColor() &&
                    (_cardMemory[(*colorMatcher_itr)].hasBeenIdentified == false))
                {
                    potentialColorMatchIndicies.push_back(*colorMatcher_itr);
                }
            }
            // std::copy_if(potentialCards.begin(), potentialCards.end(), potentialColorMatches.begin(),
            // [foundCardUnknown_itr](TableCard pot_tableCard) { return foundCardUnknown_itr->getCardFrameColor() ==
            // pot_tableCard.getCardFrameColor(); });

            int bestMatch_index = -1;
            double closestDistance = DBL_MAX;
            // TODO: could add weight to already partially blocked cards? Probably not
            for (auto potentialCards_itr = potentialColorMatchIndicies.begin();
                 potentialCards_itr != potentialColorMatchIndicies.end();
                 ++potentialCards_itr)
            {
