...
MagicCard::MagicCard(const Json::Value json_card, const CardDetails::CardSet set)
{
    ...
    _imageFilePath = ASSETS_PATH + "\\AllSets\\" + CardDetails::CardSet_name.at(set) + "\\" +
                     json_card["imageName"].asString() + ".jpg";
    _set = set;
...
void MagicCard::locateCardRegions()
{
    ...
    cv::Point brArtPoint(ulArtPoint.x + static_cast<int>(cardWidth * cardArtWidthRatio),
                         ulArtPoint.y + static_cast<int>(cardHeight * cardArtHeightRatio));
...
cv::Mat MagicCard::getFrameHistogram() const
{
    ...
    // the filter should set all the pixels in the shadow to exactly 0; should be rare that this pixel color exists in
    // real life
    cv::Mat mask;
