...
class CardDatabase
{
  public:
    ...
    std::vector<MagicCard *> returnMostAlike(MagicCard const *const cardToMatch_ptr,
                                             const int groupSize) const; // progenator for card matching
    CardDetails::FrameColor getLiveCardColor(const MagicCard *card)
        const; // In a perfect world, break this out into its own class?  Same with its digital sibling
  ...
  private:
    ...
    std::vector<MagicCard *> selectFrameColorFrom(const std::vector<MagicCard *> &fromCards,
                                                  const CardDetails::FrameColor selectColor) const;
    std::vector<MagicCard *> selectSetFrom(const std::vector<MagicCard *> &fromCards,
